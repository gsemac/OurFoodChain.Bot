...
namespace OurFoodChain.Bot.Modules {
    ...
    public class HelpModule :
        ...
        public async Task Help(string command = "", string nestedCommand = "") {
            ...
            await ShowHelp(Context, CommandHandlingService, ServiceProvider, BotConfiguration, command, nestedCommand);

        }

        public static async Task ShowHelpCategory(ICommandContext context, Discord.Services.ICommandHandlingService commandHandlingService, IServiceProvider serviceProvider, IOurFoodChainBotConfiguration botConfiguration, string commandInfoDirectory, string category) {
            ...
            await _showHelpCategory(context, commandHandlingService, serviceProvider, botConfiguration, HelpUtils.GetCommandInfoFromDirectory(commandInfoDirectory), category);

        }
        public static async Task ShowHelp(ICommandContext context, Discord.Services.ICommandHandlingService commandHandlingService, IServiceProvider serviceProvider, IOurFoodChainBotConfiguration botConfiguration, string command, string nestedCommand) {
            ...
            if (System.IO.Directory.Exists(help_directory)) {
                ...
                if (!string.IsNullOrEmpty(command)) {
                    ...
                    if (info is null)
                        ...
                        builder.AddField("Description", info.Description.Replace("\\prefix", botConfiguration.Prefix));
                        ...
                        if (info.Examples.Count() > 0) {
                            ...
                            for (int i = 0; i < info.Examples.Count(); ++i)
                                info.Examples[i] = "`" + botConfiguration.Prefix + (string.IsNullOrEmpty(nestedCommand) ? "" : command.Trim() + " ") + info.Examples[i] + "`";
                    ...
                    await _showHelpCategory(context, commandHandlingService, serviceProvider, botConfiguration, commands, "");
        ...
        public static async Task<bool> CommandIsEnabledAsync(ICommandContext context, Discord.Services.ICommandHandlingService commandHandlingService, IServiceProvider serviceProvider, string commandName) {

            CommandInfo commandInfo = commandHandlingService.GetCommandInfo(commandName);

            if (commandInfo is null)
                return false;

            if (!(await commandInfo.CheckPreconditionsAsync(context, serviceProvider)).IsSuccess)
                return false;
        ...
        private static async Task _showHelpCategory(ICommandContext context, Discord.Services.ICommandHandlingService commandHandlingService, IServiceProvider serviceProvider, IOurFoodChainBotConfiguration botConfiguration, CommandHelpInfoCollection commands, string category) {
            ...
            foreach (string cat in commands_lists.Keys) {
                ...
                foreach (CommandHelpInfo c in commands_lists[cat]) {
                    ...
                    if (await CommandIsEnabledAsync(context, commandHandlingService, serviceProvider, c.Name))
