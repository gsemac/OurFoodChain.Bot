    ...
    public class HelpCommands :
        ...
        private static async Task _showHelpCategory(ICommandContext context, List<CommandInfo> commandInfo, string category) {
            ...
            if (!string.IsNullOrEmpty(category))
                description_builder.AppendLine(string.Format("Commands listed must be prefixed with `{0}` (e.g. `{2}{0} {1}`).", 
                    category, 
                    commandInfo[0].name,
        ...
        public static async Task ShowHelp(ICommandContext context, string commandInfoDirectory, string category) {
